...
##tcsg
##
##sxhkd - демон отвечающий за горячие клавишм
#sxiv - image viever в коммандной строке (так же для скриптов)
#notify-send
#MPV - видеоплеер
#zathura - просмотр документов
#Pandoc - переделывание из одного формата в другой
#сделать универсальный скрипт "prompt" в который первым параметром передаем стрингу с вопросом что мы хотим сделать, а вторым ту команду которую мы делаем если ДА (а он автоматом должен выводить это проверять что мы нажали и если нет то завершать процесс с отменой)

#screenkey - лучший показыватель нажатых кнопок который можно забиндить на хоткей или запускать просто скриптом
#flameshot - screenchot program
##
##
## screenfetch - just fun
##
##
##
##
